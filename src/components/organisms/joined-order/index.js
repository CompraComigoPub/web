...
import { ordersJoinPending } from "utils/graphql/queries/operator";
// loading the sass style fot the component
import "./index.scss";

function JoinedOrder(props) {
  ...
  React.useEffect(
    (_) => {
      query(
        ...
        .then((list) => {
          setPublications(list.data.ordersJoinPending.orders);
          setTotalPublications(list.data.ordersJoinPending.total);
        })
        .catch((err) => {
  ...
  function onActionClick(event, item) {
    history.push("/publicacoes/" + item.id + "/participacao-na-compra");
  }
  ...
  return (
    <div className={"organism__joined-order-container " + className} {...other}>
      <div>
        <h2>Participações em compras já existentes</h2>
        <Paginator
          pageSize={5}
