...
inquirer
  ...
  .then(async answers => {
    ...
    const replacements = {
      name, kind, CamelName, upperKind, description: description || "<!-- TODO: add a description here! -->"
    };
    replacements['dashed-name'] = slugfy(name);

    const targetPath = `./src/components/${kind}s/${replacements['dashed-name']}/`;
    ...
    copydir('./bin/create/model/sample', targetPath, {
      ...
      const status = await replaceTermsInFiles({
        ...
        extensions: ['.js', '.scss', '.css', '.sass', '.tsc', 'jsx', '.mjs', '.md'],
